// rx_scene/tests/fly_camera_test.cpp -- [Phase 5 Task 5, ticket #41 row 4]
// moved from samples/09_scene/tests/test_fly_camera.cpp on promotion of
// rx::samples9::FlyCamera -> rx::scene::FlyCamera (fly_camera.h). Proves
// the W/S-inversion regression: FlyCamera's W input must displace the
// camera along its OWN camera.forward() direction, not the opposite -- the
// reported defect (W flies backward) lived in the one-line construction of
// the world-space move delta at the call site (`glm::vec3(strafe, vertical,
// -forward)`), which re-negated the `forward` accumulator a SECOND time
// even though FlyCamera::moveLocal() already multiplies it against
// camera.forward() itself (which already encodes the "-Z is local forward"
// convention via rx_scene/camera.h's own forward() accessor).
//
// Every TEST_CASE below drives the REAL production function
// (flyCameraLocalMoveDelta()) and the REAL FlyCamera::moveLocal()/
// applyLookDelta(), not a re-implementation -- a bug reintroduced at any
// call site would need to also break these assertions to pass CI silently.
#include <rx_scene/fly_camera.h>

#include <doctest/doctest.h>

using rx::scene::FlyCamera;
using rx::scene::flyCameraLocalMoveDelta;
...
TEST_CASE("FlyCamera: gamepad left-stick forward accumulator (forward = -stick.y, SDL's Y-down convention) "
          ...
          "since a caller's own per-frame update feeds both devices into one `forward` accumulator before this "
          "function ever runs") {
    FlyCamera cam;
    constexpr float kStickForwardPush = -1.0F;  // SDL: pushing the stick away from the player reports negative Y.
    const float forwardAccumulator = -kStickForwardPush;  // a caller's own `forward += -pad.leftStick.y`.
...
TEST_CASE("keyboardDrivesCamera(): true (keyboard reads allowed) when the UI does NOT claim the keyboard") {
    CHECK(keyboardDrivesCamera(/*imguiWantsKeyboard=*/false));
}

TEST_CASE("keyboardDrivesCamera(): false (keyboard reads suppressed) when the UI DOES claim the keyboard "
          "[matches a caller's pre-existing WASD/Space/LCtrl/LShift gating -- narrowed to keyboard reads only, "
